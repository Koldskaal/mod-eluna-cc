...
namespace LuaItem
{
    ...
    int GetItemLink(lua_State *L, Item *item)
    {
        ...
        oss << "|c" << std::hex << ItemQualityColors[temp->Quality] << std::dec << "|Hitem:" << temp->ItemId << ":" << item->GetEnchantmentId(PERM_ENCHANTMENT_SLOT) << ":" <<
#ifndef CLASSIC
...
#endif
            item->GetItemRandomPropertyId() << ":" << item->GetItemSuffixFactor() << ":" << (uint32)(owner ? owner->GetLevel() : 0) << "|h[" << name << "]|h|r";
    ...
    /**
     * CRAFTCRAFT get max durability
     *
     * @return uint32 maxDurability
     */
    int GetMaxDurability(lua_State *L, Item *item)
    {
        Eluna::Push(L, item->GetTemplate()->MaxDurability);
        return 1;
    }
};
#endif
